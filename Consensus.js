...
function toNamed(nodes) {
	return nodes.map(x => [x.id, x]).reduce((obj, x) => {
        obj[x[0]] = x[1];
        return obj;
    }, Object.create(null));
}
	...
	const router = {
		sendTo(orig, msg, id) {
			if (toDrop[orig]) {
				...
				if (!process.argv.includes('--nosend')) console.log(`Send: ${orig} ---(${msg})--> ${id}`);
		...
		dropNext(node) {
			const location = (typeof node === 'string' ? nodesByName[node] : node).id;
...
function createNode(location, name) {
	...
	return {
		...
		receive(author, msg) {
			if (isPaused) return;
			setTimeout(() => {
				if (process.argv.includes('--receive')) console.log(`Receive: ${author} ---(${msg})--> ${name}`);
...
function initNodesAndRouter(location, names, settings) {
	...
	nodes.forEach(x => x.init(nodes.map(a => a.id), settings));
	return {
		router: router,
		nodes: nodes,
		nn: nodesByName
